...
public fun <V : View> RecyclerView.ViewHolder.bindView(id: Int): ReadOnlyProperty<RecyclerView.ViewHolder, V> = required(id, viewFinder)
public fun <V : View> ViewContainer.bindView(id: Int): ReadOnlyProperty<ViewContainer, V> = required(id, viewFinder)
...
public fun <V : View> RecyclerView.ViewHolder.bindOptionalView(id: Int): ReadOnlyProperty<RecyclerView.ViewHolder, V?> = optional(id, viewFinder)
public fun <V : View> ViewContainer.bindOptionalView(id: Int): ReadOnlyProperty<ViewContainer, V?> = optional(id, viewFinder)
...
public fun <V : View> ViewContainer.bindViews(vararg ids: Int): ReadOnlyProperty<ViewContainer, List<V>> = required(ids, viewFinder)
...
public fun <V : View> ViewContainer.bindOptionalViews(vararg ids: Int): ReadOnlyProperty<ViewContainer, List<V>> = optional(ids, viewFinder)
...
private val ViewContainer.viewFinder: ViewContainer.(Int) -> View?
	get() = { view.findViewById(it) }
...
private fun required<T, V : View>(id: Int, finder: T.(Int) -> View?) = Lazy { t: T, desc -> t.finder(id) as V? ?: viewNotFound(id, desc) }

@Suppress("UNCHECKED_CAST")
private fun optional<T, V : View>(id: Int, finder: T.(Int) -> View?) = Lazy { t: T, desc -> t.finder(id) as V? }

@Suppress("UNCHECKED_CAST")
private fun required<T, V : View>(ids: IntArray, finder: T.(Int) -> View?) = Lazy { t: T, desc -> ids.map { t.finder(it) as V? ?: viewNotFound(it, desc) } }

@Suppress("UNCHECKED_CAST")
private fun optional<T, V : View>(ids: IntArray, finder: T.(Int) -> View?) = Lazy { t: T, desc -> ids.map { t.finder(it) as V? }.filterNotNull() }
